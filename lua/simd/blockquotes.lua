return function(M)
  ...
  function M.increment_blockquote_visual()
  ...
  function M.decrement_blockquote_visual()
    local start_line, end_line = get_visual_range()
    local lines = vim.api.nvim_buf_get_lines(0, start_line - 1, end_line, false)
    for i, line in ipairs(lines) do
  ...
  vim.keymap.set('x', prefix .. ">", M.increment_blockquote_visual, { desc = "Increment blockquote (visual)" })
  vim.keymap.set('n', prefix .. "<", M.decrement_blockquote, { desc = "Decrement blockquote" })
  vim.keymap.set('x', prefix .. "<", M.decrement_blockquote_visual, { desc = "Decrement blockquote (visual)" })
end
